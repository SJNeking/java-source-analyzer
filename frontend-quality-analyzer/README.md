...
| **Architecture** | 34 | 硬编码值、层级边界、数据流、单一职责、God 组件 |
...
### 🏗️ Architecture 规则详解 (34条)

#### 硬编码检测 (8条)
- **FE-ARCH-009**: 硬编码 API 端点
- **FE-ARCH-010**: 硬编码 URL
- **FE-ARCH-011**: 硬编码颜色值
- **FE-ARCH-012**: 硬编码配置值
- **FE-ARCH-013**: 硬编码环境变量
- **FE-ARCH-014**: 硬编码 Token/密钥 (CRITICAL)
- **FE-ARCH-015**: 硬编码文件路径
- **FE-ARCH-016**: 魔术数字

#### 架构边界与分层 (6条)
- **FE-ARCH-017**: 层级边界违规 (UI层访问数据层)
- **FE-ARCH-018**: 跨层级导入
- **FE-ARCH-019**: 组件中缺少服务层
- **FE-ARCH-020**: 领域边界违规
- **FE-ARCH-021**: 绕过集中式状态管理
- **FE-ARCH-022**: 直接修改 Store

#### 数据流清晰度 (6条)
- **FE-ARCH-023**: Prop Drilling (3+层传递)
- **FE-ARCH-024**: 双向数据流
- **FE-ARCH-025**: 状态分散
- **FE-ARCH-026**: 缺少数据源抽象
- **FE-ARCH-027**: 回调地狱/嵌套 Promise
- **FE-ARCH-028**: 隐式数据类型 (缺少 TS 类型)

#### 单一职责原则 (6条)
- **FE-ARCH-029**: God 组件 (过大/职责过多)
- **FE-ARCH-030**: 函数过长 (>50行)
- **FE-ARCH-031**: 文件过大 (>400行)
- **FE-ARCH-032**: 职责过多 (4+种职责)
- **FE-ARCH-033**: 缺少 Hook 抽象
- **FE-ARCH-034**: 混合多种数据源
